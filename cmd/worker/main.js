...
const { processTask } = require("../../internals/worker/worker");
const logger = require("../../internals/logger/logger");
...
while (true) {
    try {
        const res = await client.blPop("task_queue", 0);
        const task = JSON.parse(res.element);

        total_jobs_in_queue = await client.lLen("task_queue");
        ...
        if (retries_left > 0) {
            ...
            await client.rPush("task_queue", JSON.stringify(task));
        } else {
